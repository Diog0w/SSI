...
def alice_process(conn):
    ...
    alice_bytes = serialize_public_key(private_key.public_key())

    conn.send(alice_bytes)
    bob_bytes = conn.recv()

    bob_public_key = load_peer_public_key(bob_bytes)
...
def bob_process(conn):
    private_key = build_parameters().generate_private_key()
    bob_bytes = serialize_public_key(private_key.public_key())

    alice_bytes = conn.recv()
    conn.send(bob_bytes)

    alice_public_key = load_peer_public_key(alice_bytes)
